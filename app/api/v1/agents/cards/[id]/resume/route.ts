// POST /api/v1/agents/cards/[id]/resume
...
export async function POST(req: NextRequest, ctx: RouteContext) {
  ...
  if ((card as { status: string }).status === "canceled") {
    return NextResponse.json({ error: "cannot_resume_canceled_card" }, { status: 422 });
  }
  ...
  if (external) {
    try { await getIssuer(provider).resumeCard(external); } catch (e) {
      return NextResponse.json({ error: e instanceof Error ? e.message : "issuer resume failed" }, { status: 502 });
    }
  }

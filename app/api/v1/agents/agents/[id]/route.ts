...
import { getAgentBalance } from "@/lib/agents/zc-balances";
...
export async function GET(req: NextRequest, ctx: RouteContext) {
  ...
  // only (its amount drifts from reality, so it's never exposed).
  ...
  // REAL balance from lib/agents/zc-balances — single source of truth.
  const real = await getAgentBalance(id);
  const wallet = real
    ? {
        ...(legacyWallet ?? {}),
        id: real.zenicore_account_id,
        agent_id: id,
        balance_cents: real.balance_cents,

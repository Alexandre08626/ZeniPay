...
export function SetupWizard({ accountId, onComplete }: { accountId: string; onComplete: () => void }) {
  ...
  const submit = async () => { setSub(true); try { const r = await fetch("/api/zenipay/onboarding/submit", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ business: d.business, owner: d.owner, bank: d.bank, merchant_id: accountId }) }); const j = await r.json(); if (j.success) onComplete(); else alert(j.error||"Failed"); } catch(e) { alert(String(e)); } finally { setSub(false); } };
  return (<div>
    <h2 style={{ fontSize:20,fontWeight:900,margin:"0 0 8px",color:TX }}>Sandbox Setup</h2>
    <p style={{ fontSize:13,color:M,margin:"0 0 24px" }}>Complete all steps to go live</p>
...
export function OnboardingStatus({ accountId, onGoLive }: { accountId: string; onGoLive: () => void }) {
  ...
  return (<div style={{ maxWidth:600,margin:"0 auto" }}>
    <div style={{ textAlign:"center",padding:"40px 24px",background:"white",borderRadius:20,border:"1px solid "+B,marginBottom:24 }}>
      ...
      {st==="approved"&&<button onClick={handleGoLive} disabled={going} style={{marginTop:20,padding:"14px 32px",borderRadius:12,border:"none",background:going?"#94A3B8":G,color:"#fff",fontSize:15,fontWeight:800,cursor:"pointer"}}>{going?"Activating...":"Switch to Live Mode"}</button>}
    ...
    <div style={{ background:"white",borderRadius:16,padding:24,border:"1px solid "+B }}>
      <div style={{ fontWeight:800,marginBottom:16 }}>Onboarding Checklist</div>
